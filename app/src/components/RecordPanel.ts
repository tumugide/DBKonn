...
export class RecordPanel {
  ...
  private errorMsg = "";
  /** When true, the panel is in INSERT mode: no original, Insert button
   *  always enabled, no dirty tracking, no Delete/Revert. */
  ...
  /** Open the panel in INSERT mode with a blank/default draft row. */
  showInsert(draft: RowValue[]) {
    this.insertMode = true;
    this.record = {
      rowIndex: -1,
      original: draft.map(() => null),
      draft: draft.map((v) => cloneRowValue(v)),
      dirty: false,
    };
    this.errorMsg = "";
    this.render();
  ...
  private render() {
    ...
    const im = this.insertMode;
    const fields = this.columns
      .map((col, idx) => {
        const val = draft[idx] ?? null;
        const isNull = val === null;
        const isPk = col.is_primary_key;
        // In insert mode, PK columns are editable unless auto-generated
        const pkDisabled = im ? (isPk && isAutoGeneratedDefault(col.default_value)) : isPk;
        const disabled = ro || pkDisabled || isNull;
          ...
          !ro && col.nullable
            ? `<label class="record-null-check">
              <input type="checkbox" class="record-null-toggle" data-idx="${idx}"
                ${isNull ? "checked" : ""} ${pkDisabled ? "disabled" : ""} /> NULL
            </label>`
            : "";

        return `
          <div class="record-field ${pkDisabled ? "record-field-pk" : ""}">
    ...
    this.container.innerHTML = `
      <div class="record-panel-header">
        <span class="record-panel-title">${im ? "New Row" : "Record"}</span>
        ${ro ? '<span class="record-dirty-badge record-readonly-badge">Read-only</span>' : ""}
        ${!ro && !im && dirty ? '<span class="record-dirty-badge">Modified</span>' : ""}
      ...
      ${
        ...
        ${im
          ? '<button class="btn btn-primary" id="rp-commit">Insert</button>'
        ...
        ${!im && this.opts.onDelete ? '<button class="btn btn-danger" id="rp-delete" style="margin-left:auto">Delete</button>' : ""}
      </div>`
      }
      ${
        !ro && !im && this.columns.length > 0 && !this.columns.some((c) => c.is_primary_key)
  ...
  private syncDraftFromDom(changedIdx?: number) {
    ...
    for (const idx of indices) {
      ...
      if (col.is_primary_key && !this.insertMode) {
